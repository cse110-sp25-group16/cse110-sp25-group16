...
async function loadCards() {
    ...
    for(let i = 10; i < 11;i++) {
        ...
        if(cardDict[i].description == undefined) console.error("error desc", i, json.cards[i].name, descjson[json.cards[i].name]);

        if(cardDict[i].name == undefined) console.error("error name")
            
        if(cardDict[i].arcana == undefined) console.error("error arcana")

        if(cardDict[i].uprightMeanings == undefined) console.error("error uprightMeanings")

        if(cardDict[i].reversedMeanings == undefined) console.error("error reversedMeanings", i)
