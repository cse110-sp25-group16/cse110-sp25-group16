...
async function getCardsData() {
    const cardsData = await fetch('/source/dataparsing/cardsDataFinal.json');
...
async function init() {
    ...
    for(let i = 0; i < pulledCards.length; i++){
        ...
        cardElement.setAttribute("uprightMeanings", JSON.stringify(pulledCards[i].uprightMeanings));
        cardElement.setAttribute("reversedMeanings", JSON.stringify(pulledCards[i].reversedMeanings));
        cardElement.setAttribute("keywords", JSON.stringify(pulledCards[i].keywords));
