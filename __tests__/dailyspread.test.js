describe('Daily Spread Page', () => {
  beforeAll(async () => {
    await page.goto('http://127.0.0.1:8080/source/frontend/DailySpreadPage/dailyspread.html');
  });
  ...
  it('each tarot card should render key data in the shadow DOM', async () => {
    ...
    for (const card of cards) {
      ...
      const imgSrc = await shadow.$eval('img', img => img.getAttribute('src'));
  ...
  it('should flip the card on click', async () => {
    ...
    const wrapper = await card.evaluateHandle(el => el.shadowRoot.querySelector('.card-wrapper'));
...
it(
  'should collapse and expand the "Description" section',
  async () => {
    const card = await page.$('card-component');
    const shadow = await card.evaluateHandle(el => el.shadowRoot);
